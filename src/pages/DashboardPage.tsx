...
function FlagImg({ country }: { country: string }) {
  ...
  if (!code) return null
  return <img src={`https://flagcdn.com/20x15/${code}.png`} srcSet={`https://flagcdn.com/40x30/${code}.png 2x`}
    width={20} height={15} alt="" loading="lazy"
    style={{ borderRadius:2, flexShrink:0, boxShadow:'0 1px 3px rgba(0,0,0,.4)', display:'block', objectFit:'cover' }} />
}
...
function copyText(text: string, btn: HTMLButtonElement, orig: string) {
  navigator.clipboard.writeText(text).then(() => { btn.textContent = '✓ Copied'; setTimeout(() => { btn.textContent = orig }, 2000) })
    .catch(() => { const ta = document.createElement('textarea'); ta.value = text; ta.style.cssText = 'position:fixed;opacity:0'; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta); btn.textContent = '✓ Copied'; setTimeout(() => { btn.textContent = orig }, 2000) })
}
            ...
            <div>
              ...
              <div style={{ background:'var(--bg)', border:'1px solid var(--border)', borderRadius:4, overflow:'hidden' }}>
                {([
                  ...
                  { label:'CC', items: g.cc,  action: () => g.cc.map(r => `${r.name} <${r.email}>`).join(', ') }
                ] as const).map(({ label, items, action }) => (
...
export function DashboardPage() {
  ...
  const TOP = 132 // 60px nav + 72px filter bar

  return (
    <div>
      <TopNav onDocs={() => setSection('important')} onMails={() => setSection('emails')} />

      {/* Filter bar */}
      <div style={{ position:'fixed', top:60, left:0, right:0, background:'var(--surface)', borderBottom:'1px solid var(--border)', zIndex:90, backdropFilter:'blur(12px)' }}>
        {/* Row 1 */}
        <div style={{ display:'flex', alignItems:'center', gap:'0.5rem', padding:'0.5rem 2rem', overflowX:'auto', whiteSpace:'nowrap', scrollbarWidth:'none' }}>
          <div style={{ position:'relative', flexShrink:0 }}>
            ...
            <input type="text" placeholder="Search…" value={search}
              onChange={e => { setSearch(e.target.value.toLowerCase()); if (section !== 'sites') setSection('sites') }}
              style={{ paddingLeft:'1.8rem', paddingRight:'0.75rem', paddingTop:'0.3rem', paddingBottom:'0.3rem', background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:20, color:'var(--text)', fontFamily:'Inter,sans-serif', fontSize:'0.72rem', width:120, outline:'none' }} />
          </div>
          <span style={{ width:1, height:16, background:'var(--border)', flexShrink:0 }} />
          {(['LRDX','RR','JDX'] as Brand[]).map(b => (
            <button key={b} className={`pill-btn ${section==='sites' && brand===b ? `active-${b.toLowerCase()}` : ''}`}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'0.5rem', padding:'0.35rem 2rem', borderTop:'1px solid var(--border)', overflowX:'auto', scrollbarWidth:'none' }}>
          {['all','GCC','XM','LANGS'].map(r => (
            <button key={r} className={`pill-btn ${region===r ? 'active' : ''}`}
              style={{ padding:'0.22rem 0.75rem', fontSize:'0.68rem' }} onClick={() => setRegion(r)}>
              {r === 'all' ? 'All' : r}
            </button>
          ))}
        </div>
      ...
      {/* Main split layout */}
      <div style={{ display:'flex', alignItems:'stretch', marginTop:TOP, minHeight:`calc(100vh - ${TOP}px)` }}>

        {/* Left — site links */}
        {!leftHidden && (
          <main style={{ flex:'0 0 40%', minWidth:260, padding:'1.25rem 1.25rem 1.25rem 1.5rem', borderRight:'1px solid var(--border)', overflowY:'auto', height:`calc(100vh - ${TOP}px)`, position:'sticky', top:TOP }}>
        ...
        {/* Right — Content Tasks */}
        <div style={{ flex:1, minWidth:0, height:`calc(100vh - ${TOP}px)`, position:'sticky', top:TOP, background:'var(--bg)', display:'flex', flexDirection:'column' }}>

          {/* Toggle button */}
          <button onClick={toggleLeft} title="Toggle site links"
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
              style={{ transform: leftHidden ? 'rotate(180deg)' : 'none', transition:'transform .3s' }}>
